...
namespace TekuSP.Drivers.SHT3x
{
    ...
    public class SHT3x : DriverBaseI2C, ITemperatureSensor, IHumiditySensor, IHeaterControl, IPeriodicMeasurement, IStatusProvider
    {
        ...
        /// <inheritdoc/>
        public bool IsPeriodicMeasurementRunning
        {
            get; private set;
        }
        ...
        /// <inheritdoc cref="IPeriodicMeasurement.PeriodicMeasurementRate"/>
        public PeriodicMeasurementRate PeriodicMeasurementRate
        {
            get; private set;
        } = PeriodicMeasurementRate.EveryTwoSeconds;

        /// <inheritdoc/>
        byte IPeriodicMeasurement.PeriodicMeasurementRate => (byte)PeriodicMeasurementRate;
        ...
        {
            Status status = ReadLatestMeasurementRaw(out _, out ushort humidityTicks);
        ...
        {
            Status status = ReadLatestMeasurementRaw(out ushort temperatureTicks, out _);
            if (status != Status.SHT3x_Status_Nominal)
                return -1;
        ...
        void IPeriodicMeasurement.StartPeriodicMeasurement(byte measurementRate)
        {
            ...
            StartPeriodicMeasurement((PeriodicMeasurementRate)measurementRate);
        }
